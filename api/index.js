...
var UPLOADS_DIR = import_path.default.join(BASE_DIR, "uploads");
var DATA_DIR = import_path.default.join(BASE_DIR, "data");
var DB_FILE = import_path.default.join(DATA_DIR, "files.json");
try {
  ...
  if (!import_fs.default.existsSync(DB_FILE)) {
    import_fs.default.writeFileSync(DB_FILE, JSON.stringify([]), "utf-8");
  }
} catch (e) {
...
function writeDB(records) {
  globalThis._filesDB = records;
  try {
    const dataDir = import_path.default.dirname(DB_FILE);
    if (!import_fs.default.existsSync(dataDir)) {
      import_fs.default.mkdirSync(dataDir, { recursive: true });
    }
    import_fs.default.writeFileSync(DB_FILE, JSON.stringify(records, null, 2), "utf-8");
  } catch (err) {
...
function getCategory(mimeType, filename) {
  const ext = import_path.default.extname(filename).toLowerCase().replace(".", "");
  if (mimeType.startsWith("image/")) return "image";
  if (mimeType.startsWith("video/")) return "video";
  if (mimeType.startsWith("audio/")) return "audio";
  if (mimeType === "application/pdf" || ext === "pdf") return "pdf";
  if (["doc", "docx", "xls", "xlsx", "ppt", "pptx", "odt", "ods", "odp"].includes(ext) || mimeType.includes("word") || mimeType.includes("excel") || mimeType.includes("powerpoint") || mimeType.includes("office")) {
    return "document";
  }
...
async function getRawRequestBody(req) {
  ...
  return new Promise((resolve, reject) => {
    const chunks = [];
    const timer = setTimeout(() => resolve(Buffer.concat(chunks)), 5e3);
    req.on("data", (chunk) => {
...
function runMulterSingle(req, res, fieldName) {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      const mockStream = import_stream.Readable.from(rawBuf);
...
app.post(["/api/files/upload", "/files/upload", "/upload", "/api/upload"], async (req, res) => {
  ...
  try {
    ...
    const base64Data = size < 3 * 1024 * 1024 ? req.file.buffer.toString("base64") : void 0;
    let fileRemoteUrl = void 0;
    try {
      fileRemoteUrl = await uploadFileToCloud(req.file.buffer, originalName, mimeType) || void 0;
    } catch (e) {
    ...
    let expiresAt = null;
    if (expirationOpt === "10m") expiresAt = createdAt + 10 * 60 * 1e3;
    else if (expirationOpt === "1h") expiresAt = createdAt + 60 * 60 * 1e3;
    else if (expirationOpt === "24h") expiresAt = createdAt + 24 * 60 * 60 * 1e3;
    else if (expirationOpt === "7d") expiresAt = createdAt + 7 * 24 * 60 * 60 * 1e3;
    else if (expirationOpt === "30d") expiresAt = createdAt + 30 * 24 * 60 * 60 * 1e3;
    ...
    const storagePath = import_path.default.join(UPLOADS_DIR, `${tempId}_${safeName}`);
    try {
      if (!import_fs.default.existsSync(UPLOADS_DIR)) {
        import_fs.default.mkdirSync(UPLOADS_DIR, { recursive: true });
      }
      import_fs.default.writeFileSync(storagePath, req.file.buffer);
    } catch (e) {
...
app.get(["/api/files/:id/raw", "/files/:id/raw", "/:id/raw", "/api/:id/raw"], async (req, res) => {
  ...
  if (record.storagePath && import_fs.default.existsSync(record.storagePath)) {
    console.log(`[RAW FILE SERVED] ${id} (${record.originalName}) served from disk stream`);
    const stream = import_fs.default.createReadStream(record.storagePath);
...
app.patch(["/api/files/:id", "/files/:id", "/:id", "/api/:id"], async (req, res) => {
  ...
  if (expiration !== void 0) {
    ...
    else if (expiration === "10m") record.expiresAt = record.createdAt + 10 * 60 * 1e3;
    else if (expiration === "1h") record.expiresAt = record.createdAt + 60 * 60 * 1e3;
    else if (expiration === "24h") record.expiresAt = record.createdAt + 24 * 60 * 60 * 1e3;
    else if (expiration === "7d") record.expiresAt = record.createdAt + 7 * 24 * 60 * 60 * 1e3;
    else if (expiration === "30d") record.expiresAt = record.createdAt + 30 * 24 * 60 * 60 * 1e3;
    record.isExpired = record.expiresAt ? Date.now() > record.expiresAt : false;
  }
  if (downloadLimit !== void 0) {
...
app.delete(["/api/files/:id", "/files/:id", "/:id", "/api/:id"], async (req, res) => {
  ...
  if (record.storagePath && import_fs.default.existsSync(record.storagePath)) {
    try {
      import_fs.default.unlinkSync(record.storagePath);
    } catch (e) {
